...
export function AboutLeadership({ data }: { data?: SectionContent }) {
 ...
 {data.body && <p className="body-aroneu text-zinc-600 mb-8 whitespace-pre-line">{data.body}</p>}
 
 <div className="space-y-4 mb-8">
 {(data.features as { heading: string; body?: string }[])?.map((feature, i) => (
 <div key={i} className="flex items-center gap-4">
 <div className="w-12 h-12 rounded-full bg-zinc-200 flex items-center justify-center text-sm font-bold shrink-0">
 ...
 <div className="relative rounded-2xl overflow-hidden border border-zinc-200 transition-transform duration-300 hover:-translate-y-px">
 {data.image && (
 <Image 
 src={data.image} 
