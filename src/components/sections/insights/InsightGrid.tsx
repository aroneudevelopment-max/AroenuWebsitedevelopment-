          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {suggestedThemesData.themes?.map((theme: any, i: number) => (
                <div 
                  key={i} 
                  className="flex flex-col bg-white dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden p-8"
                >
                  <span className="text-xs font-mono uppercase px-2 py-1 rounded-md bg-zinc-100 dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 self-start mb-4">
                    Suggested topic
                  </span>
                  <h4 className="text-lg font-medium mb-3">{theme.title}</h4>
                  <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-6 flex-grow">{theme.excerpt}</p>
                  <Link href="/contact" className="text-sm font-medium text-black dark:text-white hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">
                    Discuss this topic →
                  </Link>
                </div>
              ))}
        ...
        {hasPublishedArticles && filteredArticles.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredArticles.filter(a => a.isPublished).map(article => (
              ...
              >
                <div className="relative w-full aspect-video bg-zinc-100 dark:bg-zinc-800 overflow-hidden p-6 flex items-end">
                   {/* Abstract editorial thumbnail style */}
                   <h3 className="text-lg font-medium text-black dark:text-white group-hover:text-zinc-700 dark:group-hover:text-zinc-300 transition-colors z-10 relative">
                      {article.title}
                   </h3>
                </div>
                <div className="p-8 flex flex-col flex-grow">
                  <div className="flex flex-wrap items-center gap-4 mb-4">
                    <span className="text-xs font-mono uppercase px-2 py-1 rounded-md bg-zinc-100 dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400">
                      {article.category}
                    </span>
