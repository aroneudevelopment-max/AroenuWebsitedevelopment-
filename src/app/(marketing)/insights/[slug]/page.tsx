...
export default function InsightArticlePage({ params }: { params: { slug: string } }) {
  ...
  if (!article || !article.isPublished) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] py-24 px-6 text-center">
        ...
        <p className="text-body text-zinc-600 dark:text-zinc-400 max-w-md mb-8">
  ...
  return (
    <div className="page-insight-article flex flex-col w-full">
      ...
      <div className="py-16">
        <div className="container-aroneu max-w-3xl mx-auto">
          ...
          <div className="prose prose-zinc dark:prose-invert max-w-prose">
            ...
            <div className="bg-zinc-50 dark:bg-zinc-900 p-8 rounded-xl border border-zinc-200 dark:border-zinc-800 my-12">
              <h2 className="text-h3 mt-0 mb-6">A practical checklist</h2>
              <ul className="space-y-4 m-0 p-0 list-none">
                {article.checklist.map((item, i) => (
                  <li key={i} className="flex items-start">
                    <span className="flex-shrink-0 w-6 h-6 rounded-full bg-black text-white dark:bg-white dark:text-black flex items-center justify-center mr-4 mt-0.5 text-xs">✓</span>
            ...
            <div className="border-t border-b border-zinc-200 dark:border-zinc-800 py-10 my-12">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {article.relatedReading.map((link, i) => (
                    <Link 
                      key={i} 
                      href={link.href}
                      className="group p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 transition-colors flex flex-col no-underline"
                    >
                      <span className="text-xs font-mono uppercase text-zinc-500 mb-2">{link.type}</span>
                      <span className="font-medium text-black dark:text-white group-hover:text-zinc-600 dark:group-hover:text-zinc-300 transition-colors">
                        {link.label}
                      </span>
                    </Link>
                  ))}
      ...
      <section className="py-24 bg-black text-white dark:bg-white dark:text-black border-t border-zinc-800 dark:border-zinc-200">
        <div className="container-aroneu flex flex-col items-center text-center max-w-3xl mx-auto">
          <h2 className="text-h2 mb-6 text-white dark:text-black">
            Apply this to your team model.
          </h2>
          <p className="text-body mb-10 text-zinc-300 dark:text-zinc-700">
            Talk to us about the capability you need and the governance questions you are trying to answer.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/contact" className="px-8 py-3 rounded-full text-sm font-medium transition-colors bg-white text-black hover:bg-zinc-200 dark:bg-black dark:text-white dark:hover:bg-zinc-800">
              Book a call
            </Link>
          </div>
        </div>
