...
/**
 * StepRow
 * - 1/2/3 light-weight numbered cards (no heavy filled black discs).
 * - Each card has the eyebrow + heading + description.
 * - Subtle line/dot progression is permitted; no scroll-jacking.
 */
export function StepRow({ data }: { data?: SectionContent }) {
  ...
  return (
    <section className="section-aroneu surface-paper">
      ...
      <div className="container-aroneu grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 relative">
        {items.map((item, i) => (
          <ScrollReveal
            key={i}
            direction="up"
            delay={i * 0.08}
            className="min-w-0 flex flex-col items-start p-6 border border-zinc-200 rounded-2xl surface-sand"
          >
            {/* Lightweight numbering — hairline circle, not a heavy black disc */}
