...
export function ContactWhatToInclude({ data }: { data?: SectionContent }) {
 ...
 {data.body && <p className="text-body text-zinc-600 mb-8 max-w-lg">{data.body as string}</p>}
 
 {data.items && data.items.length > 0 && (
 <ul className="space-y-4">
 {data.items.map((item, i) => (
 <li key={i} className="flex items-start">
 <span className="flex-shrink-0 w-6 h-6 rounded-full bg-primary-100 flex items-center justify-center mr-4 mt-0.5">
 <svg className="w-3.5 h-3.5 text-primary-600"fill="none"viewBox="0 0 24 24"stroke="currentColor"strokeWidth={3}>
 <path strokeLinecap="round"strokeLinejoin="round"d="M5 13l4 4L19 7"/>
 </svg>
 </span>
 <span className="text-body text-zinc-700">{item as string}</span>
 </li>
 ))}
 </ul>
 )}
 ...
 <div className="relative w-full aspect-[4/3] lg:aspect-auto lg:h-full rounded-3xl overflow-hidden border border-zinc-200">
 <Image 
 src="/images/Senior_Indian_professionals_in_w_202607030328 (1).jpeg"
 alt="Senior professionals"
 fill
 className="object-cover grayscale hover:grayscale-0 transition-all duration-700 opacity-80 mix-blend-multiply"
 />
 </div>
 </div>
 </section>
